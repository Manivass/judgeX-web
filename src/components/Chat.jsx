      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        {messages.map((message, index) => (
          <div key={index} className="flex justify-start">
            <div className="max-w-[70%] px-4 py-3 rounded-2xl bg-[#111827] border border-white/10 rounded-bl-md">
              <p className="text-xs text-purple-400 mb-1">{message.userName}</p>

              <p className="text-sm">{message.text}</p>

              <p className="text-[10px] mt-1 text-right text-gray-500">12PM</p>
            </div>
          </div>
        ))}
      </div>
